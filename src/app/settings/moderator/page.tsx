...
const Page = async () => {
    ...
    return (
        <div className="relative min-h-screen flex items-center justify-center px-4 py-16 overflow-hidden">
            ...
            <div
                className="
                    relative z-10 w-full max-w-[420px] rounded-3xl p-10
                    border border-black/[0.07] bg-black/[0.028] shadow-[0_32px_72px_rgba(0,0,0,0.12),0_0_0_1px_rgba(0,0,0,0.035)_inset]
                    dark:border-white/[0.07] dark:bg-white/[0.028] dark:shadow-[0_32px_72px_rgba(0,0,0,0.55),0_0_0_1px_rgba(255,255,255,0.035)_inset]
                    backdrop-blur-2xl
                "
            ...
            >
                ...
                <p className="text-sm text-black/30 dark:text-white/30 font-light mb-1">
                    ...
                    {sections.map((s) => (
                        <Animate key={s.href} preset="fadeUp" delay={s.delay} once>
                            <Link
                                ...
                                className="group flex items-center gap-3.5 px-4 py-3.5 rounded-2xl border border-black/[0.06] bg-black/[0.018] dark:border-white/[0.06] dark:bg-white/[0.018] hover:border-(--contrast-color)/30 hover:bg-(--contrast-color)/[0.05] transition-all duration-150"
                            >
                                <div className="flex-shrink-0 w-8 h-8 rounded-xl flex items-center justify-center bg-black/[0.04] border border-black/[0.06] dark:bg-white/[0.04] dark:border-white/[0.06] group-hover:border-(--contrast-color)/25 group-hover:bg-(--contrast-color)/10 transition-all duration-150 text-black/40 dark:text-white/40 group-hover:text-(--contrast-color)">
                                    {s.icon}
                                </div>
                                <span className="flex-1 text-sm font-medium text-black/65 dark:text-white/65 group-hover:text-black dark:group-hover:text-white transition-colors">
                                    {s.label}
                                </span>
                                <svg className="text-black/15 dark:text-white/15 group-hover:text-(--contrast-color)/50 group-hover:translate-x-0.5 transition-all duration-150" width="14" height="14" viewBox="0 0 14 14" fill="none">
                ...
                {/* Back */}
                <div className="w-full h-px bg-gradient-to-r from-transparent via-black/[0.07] to-transparent dark:via-white/[0.07] my-5"/>
                <Link
                    href={shortcuts.home}
                    className="flex items-center justify-center gap-2 w-full py-3 rounded-xl border border-black/[0.07] bg-black/[0.025] dark:border-white/[0.07] dark:bg-white/[0.025] text-sm font-medium text-black/40 dark:text-white/40 hover:text-black dark:hover:text-white hover:border-(--contrast-color)/25 hover:bg-(--contrast-color)/[0.04] transition-all duration-150"
                >
